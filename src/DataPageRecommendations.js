...
const DataPageRecommendations = (props) => {
  ...
  const [exploratoryHighRecommendationSongs, setExploratoryHighRecommendationSongs] =
    useState([]);


    const [exploratoryLowRecommendationSongs, setExploratoryLowRecommendationSongs] =
    useState([]);
  

  const getSongRecommendations = async (arrayToSet, seed_artists, seed_genres, 
    ...
    arrayToSet(recommendations.filter(item => !props.allTopSongIds.includes(item)));
  ...
  const getExploratoryLowRecommendations = async (seed_artists, seed_genres, 
    seed_tracks, target_acousticness, target_danceability, target_duration_ms, target_energy,
    target_instrumentalness,target_liveness,target_loudness,target_popularity,target_speechiness,
    target_tempo, target_valence) => {
    ...
    const maxes = inputParams.map(param => getReasonableMax(parseFloat(param)).toFixed(2));
    ...
    while (numParams > 0) {
      ...
      if (data.tracks.length >= 10) {
        ...
        setExploratoryLowRecommendationSongs(lowRecommendations.filter(item => !props.allTopSongIds.includes(item)));
  ...
  useEffect(() => {
    getSongRecommendations(setSafeRecommendationSongs, props.safeArtistIds, props.safeGenres, props.safeTrackIds,
      props.target_acousticness, props.target_danceability, props.target_duration_ms,props.target_energy,props.target_instrumentalness,props.target_liveness,props.target_loudness,props.target_popularity,props.target_speechiness,props.target_tempo,props.target_valence);
     
     
     
     
      getExploratoryHighRecommendations(props.exploratoryArtistIds, props.exploratoryGenres, props.exploratoryTrackIds,
        props.target_acousticness, props.target_danceability, props.target_duration_ms,props.target_energy,
        props.target_instrumentalness,props.target_liveness,props.target_loudness,props.target_popularity,
        props.target_speechiness,props.target_tempo,props.target_valence)

        getExploratoryLowRecommendations(props.exploratoryArtistIds, props.exploratoryGenres, props.exploratoryTrackIds,
          props.target_acousticness, props.target_danceability, props.target_duration_ms,props.target_energy,
          props.target_instrumentalness,props.target_liveness,props.target_loudness,props.target_popularity,
          props.target_speechiness,props.target_tempo,props.target_valence)
  }, []);
...
const [addingExploratoryPlaylist, setAddingExploratoryPlaylist] = useState(false);

const [addedExploratoryPlaylist, setAddedExploratoryPlaylist] = useState(false);
  ...
  const addSongsToPlaylist = async (playlist_id, type) => {
    if(type === "safe" || type === "exploratory") {
      ...
      if (type === "exploratory") {
        uris = exploratoryHighRecommendationSongs.concat(exploratoryLowRecommendationSongs).map(item => item.uri);
      }
      const { data } = await axios.post(
        `https://api.spotify.com/v1/playlists/${playlist_id}/tracks`,
        { uris },
...
const setPlaylistImage = async (playlist_id, type) => {
  try {
    ...
    await axios.put(`https://api.spotify.com/v1/playlists/${playlist_id}/images`, url, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
const [loadingExploratoryPlaylist, setLoadingExploratoryPlaylist] = useState(true);
...
{loadingSafePlaylist || addingSafePlaylist ? (
  ...
  <>
    {addedSafePlaylist ? (
      ...
      <span style={{fontWeight:'bold', fontSize:'10px', marginLeft:'2vw', color:"#18d860"}}>Added{" "}<img src={check} style={{width:'10px'}}></img></span>
    ) : (
      // Default content
      <button
...
<PlaylistCoverGenerator selectedTimeRange={props.selectedTimeRange} display_name={props.display_name}/>


      <div
        style={{ fontWeight: "bold", fontSize: "12px", marginBottom: "10px" , marginTop:"10px", maxWidth:"450px",marginLeft: 'auto',color:'gray',
        marginRight: 'auto'}}
      >
        Songs you're likely to enjoy based on your preferences for this period.
      </div>
    ...
    safeRecommendationSongs.map((song, index) => (
      ...
      >
        ...
        title="Add to your Spotify library"
          className="addToSpotifyBtn"
          onClick={() => addPlaylist(`exploratory comparify mix (${props.selectedTimeRange})`, `Songs you might not usually listen to! These have different characteristics than your typical preferences for this period.`, 'exploratory')}
          disabled={exploratoryHighRecommendationSongs.concat(exploratoryLowRecommendationSongs).length === 0}
        >
          add to{" "}
          <img
            src={spotifysmall}
      ...
      <div className="playlistCard">
        ...
        <div>
          { exploratoryHighRecommendationSongs.concat(exploratoryLowRecommendationSongs).length === 0 ? (
<div className="emptyContainer">
<span>Could not find enough recommendations.</span>
</div>
) : (
        exploratoryHighRecommendationSongs.concat(exploratoryLowRecommendationSongs).map((song, index) => (
          <div key={index} className="item">
            <img src={song.img} className="primaryImage" />
            <div className="primaryText">
              <span className="primaryName">{song.name}</span>
